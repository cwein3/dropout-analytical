...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def logprob(self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False, ret_logits=False):
        ...
        for idx in splits:
            ...
            if idx == 0:
                if self.nsplits > 1:
                    ...
                    results.append(softmaxed_head_res[:, :])
        ...
        if ret_logits:
            return results[0], head_res
        else:
            return results[0]

    def split_on_targets(self, hiddens, targets, vecs=None):
        ...
        for idx in range(self.nsplits):
            ...
            if self.nsplits == 1:
                split_targets, split_hiddens, split_vecs = [targets], [hiddens], [vecs]
                continue
            ...
            if vecs is not None:
                split_vecs.append([veci.masked_select(tmp_mask.unsqueeze(1).expand_as(veci)).view(-1, veci.size(1)) for veci in vecs])

        if vecs is None:
            return split_targets, split_hiddens
        else:
            return split_targets, split_hiddens, split_vecs

    def forward(self, weight, bias, hiddens, targets, verbose=False, return_batch=False):
        ...
        if return_batch:
            return total_loss.type_as(weight)
        else:
